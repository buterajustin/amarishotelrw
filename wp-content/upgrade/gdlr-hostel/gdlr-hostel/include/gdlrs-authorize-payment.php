	...
	(function($){
		...
		function goodlayersAuthorizeCharge(){
			...
			$.ajax({
				...
				data: { 'action':'gdlrs_hotel_authorize_payment', 'tid': tid, 'form': form_value },
				dataType: 'json',
				error: function(a, b, c){ 
					console.log(a, b, c); 

					// display error messages
					form.find('.payment-errors').text('<?php echo esc_html__('An error occurs, please refresh the page to try again.', 'gdlr-hostel'); ?>').slideDown(200);
		...
		form.submit(function(event){
			...
			if( req ){
				form.find('.payment-errors').text('<?php _e('Please fill all required fields', 'gdlr-hostel'); ?>').slideDown();
			}else{
...
<?php	
	...
	add_action( 'wp_ajax_gdlrs_hotel_authorize_payment', 'gdlrs_hotel_authorize_payment' );
	add_action( 'wp_ajax_nopriv_gdlrs_hotel_authorize_payment', 'gdlrs_hotel_authorize_payment' );
	if( !function_exists('gdlrs_hotel_authorize_payment') ){
		function gdlrs_hotel_authorize_payment(){
			...
			if( !empty($_POST['tid']) && !empty($_POST['form']) ){
				...
				$api_id = trim($hostel_option['authorize-api-id']);
				$transaction_key = trim($hostel_option['authorize-transaction-key']);
				
				$live_mode = empty($hostel_option['authorize-live-mode'])? 'enable': $hostel_option['authorize-live-mode']; 
				...
				if( empty($result->pay_amount) ){
					$ret['status'] = 'failed';
					$ret['message'] = esc_html__('Cannot retrieve pricing data, please try again.', 'gdlr-hostel');
					...
					try{
						...
						if( $response != null ){
						    ...
						    if( ($tresponse != null) && ($tresponse->getResponseCode() == '1') ){
								...
								do_action('gdlrs_update_transaction_availability', $_POST['tid']);
						        ...
						        $ret['status'] = 'failed';
						    	$ret['message'] = esc_html__('Cannot charge credit card, please check your card credentials again.', 'gdlr-hostel');
						    ...
						    $ret['status'] = 'failed';
						    $ret['message'] = esc_html__('No response returned, please try again.', 'gdlr-hostel');
						}
						$ret['data'] = $_POST;
